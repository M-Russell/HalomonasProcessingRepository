...
Data were acquired as MS^e^ in which the instrument alternates between a low energy scan and a collision energy "ramp".
The low energy scan captures precursor ions, the ramp captures product ion spectra.
Data of this kind is are supported by opensource tools such as `openms` or `trans-proteomics pipeline` for spectra generation.
So the Waters proprietary tools are used.

The proprietary tools that manage the pipeline are as follows.
...
After peak-picking data are searched and processed along two separate tracks.
...
The analysis pipeline is executed using `gnu-make` also called `make`.
...
The principle of `make` is that the program is supplied with a set of "rules" for converting "prerequisites" into "targets" by means of a "recipe" (https://www.gnu.org/software/make/manual/html_node/Rule-Introduction.html#Rule-Introduction).
...
```{bash, engine='bash', echo=TRUE, eval=FALSE, size='scriptsize'}
target : prerequisite
...
*Whole Cell Lysate Data:*

```{bash mk_rawPaths_wcl, engine='bash', echo=TRUE, eval=FALSE, indent=" "}
...
```{bash mk_rawPaths_peri, engine='bash', echo=TRUE, eval=FALSE,  indent=" ", tidy=TRUE}
...
```{bash mk_rawPaths_sec, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_rawPaths_int, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_intPathDir, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
After defining the directories the intermediate files for the outputs of the Waters executables are listed: 

```{bash mk_intPathsWaters, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}

# Apex paths:
apexPaths:=$(addprefix ${apexDir},\
						$(notdir $(rawFiles_All:.raw/_HEADER.TXT=_Apex3D.bin)))
# $(info first apex $(firstword ${apexPaths}))

# Apex paths WCL:
apexPaths_wcl:= $(addprefix ${apexDir},\
					$(notdir $(rawFiles_All_wcl:.raw/_HEADER.TXT=_Apex3D.bin)))

# Apex paths periplasm:
apexPaths_peri:=$(addprefix ${apexDir},\
					$(notdir $(rawFiles_All_peri:.raw/_HEADER.TXT=_Apex3D.bin)))

# Apex paths periplasm:
apexPaths_sec:=$(addprefix ${apexDir},\
					$(notdir $(rawFiles_All_sec:.raw/_HEADER.TXT=_Apex3D.bin)))

# Pep3d Paths:
pep3dPaths:=$(addprefix ${pep3dDir},\
					$(notdir $(rawFiles_All:.raw/_HEADER.TXT=_Pep3D_Spectrum.bin)))
...
#IADB paths
## Merged files:
iadb_mg_bin:=$(abspath $(addprefix ${iadbDir},\
			$(notdir $(pep3dMergedFiles:_Pep3D_Spectrum.bin=_IA_workflow.bin))))
iadb_mg_tabFrag:=$(abspath $(addprefix ${iadbTabDir},\
			$(notdir $(pep3dMergedFiles:_Pep3D_Spectrum.bin=_IA_final_fragment.csv))))
iadb_mg_tabPep:=$(abspath $(addprefix ${iadbTabDir},\
			$(notdir $(pep3dMergedFiles:_Pep3D_Spectrum.bin=_IA_final_peptide.csv))))
iadb_mg_tabProt:=$(abspath $(addprefix ${iadbTabDir},\
			$(notdir $(pep3dMergedFiles:_Pep3D_Spectrum.bin=_IA_final_protein.csv))))

## All IADB files:
iadb_all:=$(sort ${iadb_mg_bin} ${iadb_mg_tabFrag} ${iadb_mg_tabPep} ${iadb_mg_tabProt})
```

The intermediate files for the trans proteomie pipeline part are also listed:

```{bash mk_intPathsTPP, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}

# mzXML
pep3dEcoilPaths_mzXML:=$(addprefix ./pep3d/,\
					$(notdir $(rawFiles_Ecoli:.raw/_HEADER.TXT=_Pep3D_Spectrum.mzXML)))
pep3dHaloPaths_mzXML:=$(addprefix ./pep3d/,\
				$(notdir $(rawFiles_Halo:.raw/_HEADER.TXT=_Pep3D_Spectrum.mzXML)))
...
# pepXML
pep3dEcoliPaths_pepXML:=$(addprefix ./pep3d/,\
					$(notdir $(rawFiles_Ecoli:.raw/_HEADER.TXT=_Pep3D_Spectrum.pep.xml)))
pep3dHaloPaths_pepXML:=$(addprefix ./pep3d/,\
					$(notdir $(rawFiles_Halo:.raw/_HEADER.TXT=_Pep3D_Spectrum.pep.xml)))
...
# prophet
all_prophet:=./pep3d/interact_all_ecoli_prophet.pep.xml \
...
The spectral libraries are output in various formats:

```{bash mk_libraries, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_fastaPaths, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
Other targets may be spesified on the gnu-make invoking comand line, the targets `build[Step]` for example `buildApex` are provided for trouble shooting to enable each step of the process to be built in turn.
For more detailed trouble shooting `testBuild[step]` targets such as `testBuildApex` build a single example target to test the recipy on a single example.
...
```{bash mk_make_special, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_makeFasta, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
The Apex3d program has many settings.
These are set for each of the sets of files as below:

*Whole Cell Lysate Settings:*

```{bash mk_apexSettings_wcl, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_apexSettings_peri, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_apexSettings_sec, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
As with ``Apex3d` `Peptide3d` has a many settings.
A few of these were adjusted during pipeline development and are set here:

```{bash mk_pep3d, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_apexRun_wcl, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_apexRun_peri, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_apexRun_sec, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
The result is that only a single `pep3d` file may be produced at a time, which prevents multiple concurent instances of `peptide3d` consuming all available RAM and silently crashing the process.


```{bash mk_pep3dRun, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_mergePep3dIni_wcl, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{bash mk_search, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
The trans-proteomic pipeline (TPP) is used to process `.mgf` files produced by peptide3d above into a spectral library.
...
```{r mk_convertpep3dTo_pepXML, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
For each `mzxml` file a copy is made of the standard X!tandem paramater file.
...
```{r mk_make_xtandem, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{r mk_make_tandem2xml, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
The `pepXML` files are then combined into single `pepXML` file using TPP `xinteract`:


```{r mk_make_xinteract, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{r mk_make_peptideProphet, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{r mk_mayu_prophet, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
The double-dollar "\$\$" marked `make` functions (see ["Using Variables in Recipes"](https://www.gnu.org/software/make/manual/html_node/Variables-in-Recipes.html) are executed during recipe run.
In this case the ["eval"](https://www.gnu.org/software/make/manual/html_node/Eval-Function.html) function defines `mayuCutoff` as the lowest identification probability in the 5% protein FDR mayu output table; that value is printed to the file `mayuCutoff.txt`; which in turn is read by spectrast to set the minimum probability against which to include a spectrum. 


```{r mk_tpp_spectrast, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{r mk_tpp_spectrast2tsvOS, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
```{r mk_tpp_spectrast2tsvPK, engine='bash', echo=TRUE, eval=FALSE, indent=" ", tidy=TRUE}
...
That concludes the documented make file.
`Make` is sometimes a difficult scripting program to work with, however it does provide a comprehensive toolkit for managing bioinformatics pipelines as well as the program compilation tasks for which it was designed.
Here each of the steps in a pipelline from `.raw` data to product output are presented in a way that allows repeatable analysis; review of processing steps; and re-use of code. 
